...
type
     ...
     Line_Node = record
       data: array[1..MAX_COUNT_OF_COLUMNS] of Cell; // строка из €чеек
...
implementation
  ...
  {ƒобавл€ет линию таблицы в список.
  ¬ процессе создаетс€ копи€(list_copy) свойства класса Line,
  затем копии присваиваетс€ адрес параметра записи "next" до момента,
  пока не будет найден последний элемент списка.
  ѕосле нахождени€ в конец становитс€
  ...
  {¬озвращает строку таблицы с определенным номером.
  —оздаетс€ копи€ (line_copy) свойства класс Line ей передаетс€ ссылка на следующий элемент,
  ...
  begin
    ...
    begin
      ...
      elm^.data[i].border.Destroy; // удал€етс€ стара€ граница
      elm^.data[i].Destroy;
    end;
    _pullOffElmFromList(elm); // вытаскивает элемент из списка
    _renumberList; // пересчитывает список без этого элемента
    insert(newEmptyElm, newEmptyElm^.number);  // вставл€ет элемент в список
  end;
