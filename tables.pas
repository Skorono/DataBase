...
const
  preliminaryTextLen = 6;
var
  selectionDescription: TextButton;
  selectionMenu: SwitchMenu;
begin
  selectionMenu := createSelectionMenu;
  selectionMenu.addButton('Êàìåííûé');
  selectionMenu.addButton('Äåğåâÿííûé');
  selectionMenu.addButton('Êàğêàñíûé');
  additional_textbutton[length(additional_textbutton)-1].clear;

  selectionDescription := TextButton.Init(preliminaryTextLen, 1, selectionMenu.x - preliminaryTextLen, selectionMenu.y, 0, 'Type: '); // ñîçäàåì òåêñò ïåğåä ìåíş
